...
struct LEVELDB_EXPORT Options {
   ...
   * 该新的version相对于老的version，删除了被合并的(sstable)文件，添加合并后的(sstable)文件。
   *   由于block cache中的cache id代表(sstable)文件id，所以compaction更新后，由于旧(sstable)文件已经删除，
   * 所以不会再去访问该旧(sstable)文件对应的cache。而访问到新(sstable)文件时，由于该(sstable)文件在block cache中没有缓存，
   * 则会创建一个新的cache放入block_cache中。无论如何都访问不到旧的cache
   *   具体可参考代码：
